...
import {
  ...
  ArrowUpDown, ArrowUp, ArrowDown, Eye, Receipt
} from 'lucide-react'
...
export default function PurchaseReport() {
  ...
  const sortedPurchases = useMemo(() => {
    const sorted = [...filteredPurchases].sort((a, b) => {
      ...
      switch (sortField) {
        case 'billing_date':
          valA = a.billing_date || a.purchase_date || ''
          valB = b.billing_date || b.purchase_date || ''
          break
        case 'invoice_number':
        ...
        case 'subtotal':
          ...
          break
        case 'vat_amount':
          valA = Number(a.vat_amount) || 0
          valB = Number(b.vat_amount) || 0
          break
        default:
          valA = a.billing_date || a.purchase_date || ''
          valB = b.billing_date || b.purchase_date || ''
  ...
  const summaryStats = useMemo(() => {
    ...
    // Item-level stats (only for loaded items)
    let totalBottles = 0
    let totalCases = 0
    let totalBulkLiters = 0
    let itemCostSum = 0
    ...
    return {
      totalPurchases, totalAmount, totalSubtotal, totalVat, totalCess,
      totalSpecial, totalTcs, uniqueSuppliers, receivedCount, returnCount,
      totalBottles, totalCases, totalBulkLiters, itemCostSum
    }
  ...
  const bySupplier = useMemo(() => {
    ...
    filteredPurchases.forEach(p => {
      ...
      map[key].count++
      map[key].subtotal += Number(p.subtotal) || 0
      map[key].vat += Number(p.vat_amount) || 0
      map[key].cess += Number(p.cess_amount) || 0
      map[key].special += Number(p.special_amount) || 0
      map[key].tcs += Number(p.tcs_amount) || 0
      map[key].total += Number(p.total_amount) || 0
    })
  ...
  const byCategory = useMemo(() => {
    ...
    filteredPurchases.forEach(p => {
      ...
      if (items.length === 0 && (filterCategory || filterProduct || filterBrand)) {
        // Already filtered at purchase level
      }
      items.forEach(item => {
        const key = item.category || 'Uncategorized'
        if (!map[key]) map[key] = { name: key, count: 0, bottles: 0, cases: 0, cost: 0, bulkLiters: 0 }
        map[key].count++
        map[key].bottles += item.total_bottles || 0
        map[key].cases += item.qty_cases || 0
        map[key].cost += item.total_cost || 0
        map[key].bulkLiters += item.qty_bulk_liters || 0
      })
    })
    return Object.values(map).sort((a, b) => b.cost - a.cost)
  }, [filteredPurchases, expandedItems, filterCategory, filterProduct, filterBrand])
      ...
      items.forEach(item => {
        ...
        if (!map[key]) {
          map[key] = {
            ...
            bottle_size: item.bottle_size || '',
            mrp: item.mrp,
            count: 0, bottles: 0, cases: 0, cost: 0,
            bulkLiters: 0, lpLiters: 0
          }
        }
        map[key].count++
        map[key].bottles += item.total_bottles || 0
        ...
        map[key].cost += item.total_cost || 0
        map[key].bulkLiters += item.qty_bulk_liters || 0
        map[key].lpLiters += item.qty_lp_liters || 0
      })
    })
    return Object.values(map).sort((a, b) => b.cost - a.cost)
  }, [filteredPurchases, expandedItems])
  ...
  const handleExportExcel = () => {
    ...
    const summaryData = filteredPurchases.map(p => ({
    ...
    ws1['!cols'] = [
      { wch: 20 }, { wch: 14 }, { wch: 14 }, { wch: 20 }, { wch: 16 },
    ...
    filteredPurchases.forEach(p => {
      ...
      if (items.length === 0) {
        itemData.push({
          ...
          'Supplier': p.supplier_name || '',
          'Product': '—',
          'Brand': '', 'Category': '', 'Bottle Size': '',
          'Cases': '', 'Total Bottles': '', 'Bulk Liters': '',
          'MRP': '', 'Rate/Case': '', 'Rate/Unit': '', 'Total Cost': ''
        })
      } else {
        items.forEach(item => {
          ...
          itemData.push({
            ...
            'Cases': cases,
            'Total Bottles': totalBtls,
            'Bulk Liters': item.qty_bulk_liters || 0,
            'MRP': item.mrp || 0,
    ...
    const productData = byProduct.map(p => ({
      ...
      'MRP': p.mrp,
      'Times Purchased': p.count,
      'Total Cases': p.cases,
      'Total Bottles': p.bottles,
      'Bulk Liters': p.bulkLiters,
      'L.P. Liters': p.lpLiters,
      'Total Cost': p.cost,
    }))
  ...
  const handleExportExcelFull = async () => {
    // Ensure all items are loaded before export
    ...
    if (missing.length > 0) {
      ...
      // Use a timeout to allow state to update before generating the file
    ...
    <div className="space-y-6">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={clsx(
              ...
              showFilters && "bg-blue-50 dark:bg-blue-900/30 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400"
            )}
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        <div className="card p-3 border dark:border-slate-700">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] uppercase font-semibold text-gray-400 dark:text-gray-500 tracking-wide">Total Purchases</span>
          </div>
        ...
        <div className="card p-3 border dark:border-slate-700">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] uppercase font-semibold text-gray-400 dark:text-gray-500 tracking-wide">Total Amount</span>
          </div>
        ...
        <div className="card p-3 border dark:border-slate-700">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] uppercase font-semibold text-gray-400 dark:text-gray-500 tracking-wide">Subtotal</span>
          </div>
        ...
        <div className="card p-3 border dark:border-slate-700">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] uppercase font-semibold text-gray-400 dark:text-gray-500 tracking-wide">Suppliers</span>
          </div>
        ...
        <div className="card p-3 border dark:border-slate-700">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] uppercase font-semibold text-gray-400 dark:text-gray-500 tracking-wide">Total Bottles</span>
          </div>
        ...
        <div className="card p-3 border dark:border-slate-700">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] uppercase font-semibold text-gray-400 dark:text-gray-500 tracking-wide">VAT + CESS</span>
          </div>
          <p className="text-lg font-bold text-amber-600 dark:text-amber-400">
            ₹{(summaryStats.totalVat + summaryStats.totalCess + summaryStats.totalSpecial + summaryStats.totalTcs).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </p>
        </div>
      </div>
        ...
        <div className="card p-4 border dark:border-slate-700 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-200 flex items-center gap-2">
              <Filter className="w-4 h-4 text-blue-500" /> Report Filters
            </h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
            ...
            {/* Supplier */}
            <div>
              <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wide">Supplier</label>
              <select
            ...
            {/* Invoice */}
            <div>
              <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wide">Invoice No.</label>
              <input
                type="text"
            ...
            {/* Brand */}
            <div>
              <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wide">Brand</label>
              <input
                type="text"
            ...
            {/* Category */}
            <div>
              <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wide">Category</label>
              <input
                type="text"
            ...
            {/* Vehicle */}
            <div>
              <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wide">Vehicle No.</label>
              <input
                type="text"
            ...
            {/* Min Amount */}
            <div>
              <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wide">Min Amount ₹</label>
              <input
                type="number"
                placeholder="0"
      ...
      <div className="border-b border-gray-200 dark:border-gray-700">
        <nav className="flex gap-1 overflow-x-auto -mb-px">
          {[
            { id: 'detail', label: 'Purchase Detail', icon: Receipt },
            { id: 'bySupplier', label: 'By Supplier', icon: Building2 },
            { id: 'byCategory', label: 'By Category', icon: BarChart3 },
            { id: 'byBrand', label: 'By Brand', icon: Package },
            { id: 'byProduct', label: 'By Product', icon: ShoppingBag },
          ].map(tab => (
            <button
              key={tab.id}
      ...
      {loading ? (
        ...
        <div className="card p-10 text-center border dark:border-slate-700">
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500">
            {hasActiveFilters ? 'No purchases match the selected filters' : 'No purchases recorded yet'}
          </p>
        </div>
      ) : (
        <>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-gray-100 dark:border-gray-800 bg-slate-50 dark:bg-slate-800/50">
                        <th className="p-2.5 text-left font-semibold cursor-pointer select-none hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors" onClick={() => handleSort('invoice_number')}>
                          <div className="flex items-center gap-1">Invoice {getSortIcon(sortField === 'invoice_number' ? sortDir : '')}</div>
                        </th>
                        <th className="p-2.5 text-left font-semibold cursor-pointer select-none hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors" onClick={() => handleSort('billing_date')}>
                          <div className="flex items-center gap-1">Date {getSortIcon(sortField === 'billing_date' ? sortDir : '')}</div>
                        </th>
                        <th className="p-2.5 text-left font-semibold cursor-pointer select-none hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors" onClick={() => handleSort('supplier_name')}>
                          <div className="flex items-center gap-1">Supplier {getSortIcon(sortField === 'supplier_name' ? sortDir : '')}</div>
                        </th>
                        <th className="p-2.5 text-center font-semibold">Vehicle</th>
                        <th className="p-2.5 text-right font-semibold cursor-pointer select-none hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors" onClick={() => handleSort('subtotal')}>
                          <div className="flex items-center justify-end gap-1">Subtotal {getSortIcon(sortField === 'subtotal' ? sortDir : '')}</div>
                        </th>
                        <th className="p-2.5 text-right font-semibold cursor-pointer select-none hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors" onClick={() => handleSort('vat_amount')}>
                          <div className="flex items-center justify-end gap-1">VAT {getSortIcon(sortField === 'vat_amount' ? sortDir : '')}</div>
                        </th>
                        <th className="p-2.5 text-right font-semibold cursor-pointer select-none hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors" onClick={() => handleSort('total_amount')}>
                          <div className="flex items-center justify-end gap-1">Total {getSortIcon(sortField === 'total_amount' ? sortDir : '')}</div>
                        </th>
                      ...
                      {paginatedPurchases.map(p => (
                        <>
                          <tr key={p.id} className={clsx(
                            ...
                            p.status === 'RETURN' && "bg-red-50/30 dark:bg-red-900/10"
                          )}>
                            <td className="p-2.5">
                              <div className="font-mono font-semibold text-gray-800 dark:text-gray-200">{p.invoice_number}</div>
                              {p.invoice_date && (
                                <div className="text-[10px] text-gray-400">Inv: {format(new Date(p.invoice_date + 'T00:00:00'), 'dd MMM yy')}</div>
                              )}
                            </td>
                            ...
                            <td className="p-2.5">
                              <div className="font-medium text-gray-800 dark:text-gray-200">{p.supplier_name || '—'}</div>
                              {p.notes && <div className="text-[10px] text-gray-400">Pass: {p.notes}</div>}
                            </td>
                                ...
                                <table className="w-full text-xs">
                                  <thead>
                                    <tr className="text-gray-400 border-b border-gray-200 dark:border-gray-700">
                                      ...
                                      <th className="text-center py-1.5 font-medium w-16">Size</th>
                                      <th className="text-center py-1.5 font-medium w-12">MRP</th>
                                      <th className="text-center py-1.5 font-medium w-14">Cases</th>
                                      <th className="text-center py-1.5 font-medium w-16 font-bold text-green-500">Bottles</th>
                                      <th className="text-center py-1.5 font-medium w-14">Bulk L</th>
                                      <th className="text-right py-1.5 font-medium w-18 text-orange-500">Rate/Case</th>
                                      <th className="text-right py-1.5 font-medium w-18 text-green-500">Rate/Unit</th>
                                  ...
                                  <tbody>
                                    {expandedItems[p.id].map((item, idx) => {
                                      ...
                                      return (
                                        <tr key={item.id} className="border-b border-gray-100 dark:border-gray-800/50 hover:bg-white/50 dark:hover:bg-gray-800/30">
                                          ...
                                          <td className="py-1.5">
                                            ...
                                            {item.brand && <div className="text-[10px] text-gray-400">{item.brand}</div>}
                                          </td>
                                          <td className="text-center py-1.5 text-gray-500">{item.bottle_size || '-'}</td>
                                          <td className="text-center py-1.5">₹{item.mrp}</td>
                                          <td className="text-center py-1.5 font-medium">{cases || '-'}</td>
                                          <td className="text-center py-1.5 font-bold text-green-600 dark:text-green-400">{totalBtls || '-'}</td>
                                          <td className="text-center py-1.5 text-gray-500">{item.qty_bulk_liters || '-'}</td>
                                          <td className="text-right py-1.5 font-semibold text-orange-600 dark:text-orange-400">₹{ratePerCase.toFixed(2)}</td>
                                          <td className="text-right py-1.5 font-semibold text-green-600 dark:text-green-400">₹{ratePerUnit.toFixed(2)}</td>
                                          <td className="text-right py-1.5 font-medium text-blue-700 dark:text-blue-300">₹{item.total_cost.toFixed(2)}</td>
                                        </tr>
                                      )
                                    })}
                                    {/* Item totals row */}
                                    <tr className="bg-slate-100/50 dark:bg-slate-800/50 font-semibold">
                                      <td colSpan={4} className="py-1.5 text-right text-gray-500">Item Total</td>
                                      <td className="py-1.5 text-center">{expandedItems[p.id].reduce((s, i) => s + (i.qty_cases || 0), 0)}</td>
                                      <td className="py-1.5 text-center text-green-600">{expandedItems[p.id].reduce((s, i) => s + (i.total_bottles || 0), 0)}</td>
                                      <td className="py-1.5 text-center">{expandedItems[p.id].reduce((s, i) => s + (i.qty_bulk_liters || 0), 0).toFixed(1)}</td>
                                      <td colSpan={2}></td>
                                      <td className="py-1.5 text-right text-blue-700 dark:text-blue-300">₹{expandedItems[p.id].reduce((s, i) => s + (i.total_cost || 0), 0).toFixed(2)}</td>
                                    </tr>
                                  </tbody>
                                </table>
                              </td>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-2">
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Showing {(currentPage - 1) * ITEMS_PER_PAGE + 1}–{Math.min(currentPage * ITEMS_PER_PAGE, sortedPurchases.length)} of {sortedPurchases.length}
                  </p>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-gray-100 dark:border-gray-800 bg-slate-50 dark:bg-slate-800/50">
                      ...
                      <th className="p-3 text-center font-semibold">Purchases</th>
                      <th className="p-3 text-right font-semibold">Subtotal</th>
                      <th className="p-3 text-right font-semibold">VAT</th>
                      <th className="p-3 text-right font-semibold">CESS</th>
                      <th className="p-3 text-right font-semibold">Special</th>
                      <th className="p-3 text-right font-semibold">TCS</th>
                  ...
                  <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                    {bySupplier.map((s, i) => {
                      const pct = summaryStats.totalAmount > 0 ? (s.total / summaryStats.totalAmount * 100) : 0
                      return (
                        <tr key={i} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                          ...
                          <td className="p-3 text-center">
                            <span className="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded-full font-semibold">{s.count}</span>
                          </td>
                          <td className="p-3 text-right">₹{s.subtotal.toFixed(2)}</td>
                          <td className="p-3 text-right text-gray-500">₹{s.vat.toFixed(2)}</td>
                          <td className="p-3 text-right text-gray-500">₹{s.cess.toFixed(2)}</td>
                          <td className="p-3 text-right text-gray-500">₹{s.special.toFixed(2)}</td>
                          <td className="p-3 text-right text-gray-500">₹{s.tcs.toFixed(2)}</td>
                          <td className="p-3 text-right font-bold text-blue-700 dark:text-blue-300">₹{s.total.toFixed(2)}</td>
                  ...
                  <tfoot className="bg-slate-50 dark:bg-slate-800/50 border-t-2 border-slate-300 dark:border-slate-600">
                    <tr className="font-bold">
                      <td colSpan={2} className="p-3 text-right text-gray-700 dark:text-gray-300">Grand Total</td>
                      <td className="p-3 text-center">{summaryStats.totalPurchases}</td>
                      <td className="p-3 text-right">₹{summaryStats.totalSubtotal.toFixed(2)}</td>
                      <td className="p-3 text-right">₹{summaryStats.totalVat.toFixed(2)}</td>
                      <td className="p-3 text-right">₹{summaryStats.totalCess.toFixed(2)}</td>
                      <td className="p-3 text-right">₹{summaryStats.totalSpecial.toFixed(2)}</td>
                      <td className="p-3 text-right">₹{summaryStats.totalTcs.toFixed(2)}</td>
                      <td className="p-3 text-right text-blue-700 dark:text-blue-300">₹{summaryStats.totalAmount.toFixed(2)}</td>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-gray-100 dark:border-gray-800 bg-slate-50 dark:bg-slate-800/50">
                      ...
                      <th className="p-3 text-center font-semibold text-green-600">Bottles</th>
                      <th className="p-3 text-center font-semibold">Bulk L</th>
                      <th className="p-3 text-right font-semibold text-blue-600">Total Cost</th>
                      <th className="p-3 text-right font-semibold">%</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                    {byCategory.map((c, i) => {
                      const pct = summaryStats.itemCostSum > 0 ? (c.cost / summaryStats.itemCostSum * 100) : 0
                      return (
                        <tr key={i} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                          <td className="p-3 text-gray-400">{i + 1}</td>
                          <td className="p-3 font-medium text-gray-800 dark:text-gray-200">{c.name}</td>
                          <td className="p-3 text-center">
                            <span className="bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 px-2 py-0.5 rounded-full font-semibold">{c.count}</span>
                          </td>
                          <td className="p-3 text-center">{c.cases}</td>
                          <td className="p-3 text-center font-bold text-green-600 dark:text-green-400">{c.bottles}</td>
                          <td className="p-3 text-center text-gray-500">{c.bulkLiters.toFixed(1)}</td>
                          <td className="p-3 text-right font-bold text-blue-700 dark:text-blue-300">₹{c.cost.toFixed(2)}</td>
                          <td className="p-3 text-right">
                            <div className="flex items-center gap-2 justify-end">
                              <div className="w-16 h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                  ...
                  <tfoot className="bg-slate-50 dark:bg-slate-800/50 border-t-2 border-slate-300 dark:border-slate-600">
                    <tr className="font-bold">
                      ...
                      <td className="p-3 text-center">{byCategory.reduce((s, c) => s + c.count, 0)}</td>
                      <td className="p-3 text-center">{byCategory.reduce((s, c) => s + c.cases, 0)}</td>
                      <td className="p-3 text-center text-green-600">{byCategory.reduce((s, c) => s + c.bottles, 0)}</td>
                      <td className="p-3 text-center">{byCategory.reduce((s, c) => s + c.bulkLiters, 0).toFixed(1)}</td>
                      <td className="p-3 text-right text-blue-700 dark:text-blue-300">₹{byCategory.reduce((s, c) => s + c.cost, 0).toFixed(2)}</td>
                      <td className="p-3 text-right text-[10px] text-gray-400">100%</td>
                    </tr>
                  </tfoot>
                </table>
            ...
            <div className="card overflow-hidden border dark:border-slate-700 shadow-sm">
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  ...
                  <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                    {byBrand.map((b, i) => {
                      const pct = summaryStats.itemCostSum > 0 ? (b.cost / summaryStats.itemCostSum * 100) : 0
                      return (
                        <tr key={i} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                          <td className="p-3 text-gray-400">{i + 1}</td>
                          <td className="p-3 font-medium text-gray-800 dark:text-gray-200">{b.name}</td>
                          <td className="p-3 text-center">
                            <span className="bg-cyan-50 dark:bg-cyan-900/20 text-cyan-600 dark:text-cyan-400 px-2 py-0.5 rounded-full font-semibold">{b.count}</span>
                          </td>
                          <td className="p-3 text-center">{b.cases}</td>
                          <td className="p-3 text-center font-bold text-green-600 dark:text-green-400">{b.bottles}</td>
                          <td className="p-3 text-center text-gray-500">{b.bulkLiters.toFixed(1)}</td>
                          <td className="p-3 text-right font-bold text-blue-700 dark:text-blue-300">₹{b.cost.toFixed(2)}</td>
                          <td className="p-3 text-right">
                            <div className="flex items-center gap-2 justify-end">
                              <div className="w-16 h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                  ...
                  <tfoot className="bg-slate-50 dark:bg-slate-800/50 border-t-2 border-slate-300 dark:border-slate-600">
                    <tr className="font-bold">
                      <td colSpan={2} className="p-3 text-right text-gray-700 dark:text-gray-300">Total</td>
                      <td className="p-3 text-center">{byBrand.reduce((s, b) => s + b.count, 0)}</td>
                      <td className="p-3 text-center">{byBrand.reduce((s, b) => s + b.cases, 0)}</td>
                      <td className="p-3 text-center text-green-600">{byBrand.reduce((s, b) => s + b.bottles, 0)}</td>
                      <td className="p-3 text-center">{byBrand.reduce((s, b) => s + b.bulkLiters, 0).toFixed(1)}</td>
                      <td className="p-3 text-right text-blue-700 dark:text-blue-300">₹{byBrand.reduce((s, b) => s + b.cost, 0).toFixed(2)}</td>
                      <td className="p-3 text-right text-[10px] text-gray-400">100%</td>
                    </tr>
                  </tfoot>
                </table>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-gray-100 dark:border-gray-800 bg-slate-50 dark:bg-slate-800/50">
                      ...
                      <th className="p-3 text-left font-semibold">Brand</th>
                      <th className="p-3 text-center font-semibold">Size</th>
                      <th className="p-3 text-center font-semibold">MRP</th>
                      <th className="p-3 text-center font-semibold">Times</th>
                      <th className="p-3 text-center font-semibold">Cases</th>
                      <th className="p-3 text-center font-semibold text-green-600">Bottles</th>
                      <th className="p-3 text-center font-semibold">Bulk L</th>
                      <th className="p-3 text-center font-semibold">LP L</th>
                      <th className="p-3 text-right font-semibold text-blue-600">Total Cost</th>
                      <th className="p-3 text-right font-semibold">%</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                    {byProduct.map((p, i) => {
                      const pct = summaryStats.itemCostSum > 0 ? (p.cost / summaryStats.itemCostSum * 100) : 0
                      return (
                        <tr key={i} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                          <td className="p-3 text-gray-400">{i + 1}</td>
                          <td className="p-3 font-medium text-gray-800 dark:text-gray-200">{p.name}</td>
                          <td className="p-3 text-gray-500 dark:text-gray-400">{p.brand || '-'}</td>
                          <td className="p-3 text-center text-gray-500">{p.bottle_size || '-'}</td>
                          <td className="p-3 text-center">₹{p.mrp}</td>
                          <td className="p-3 text-center">
                            <span className="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded-full font-semibold">{p.count}</span>
                          </td>
                          <td className="p-3 text-center">{p.cases}</td>
                          <td className="p-3 text-center font-bold text-green-600 dark:text-green-400">{p.bottles}</td>
                          <td className="p-3 text-center text-gray-500">{p.bulkLiters.toFixed(1)}</td>
                          <td className="p-3 text-center text-gray-500">{p.lpLiters.toFixed(1)}</td>
                          <td className="p-3 text-right font-bold text-blue-700 dark:text-blue-300">₹{p.cost.toFixed(2)}</td>
                          <td className="p-3 text-right">
                            <div className="flex items-center gap-2 justify-end">
                              <div className="w-16 h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                  ...
                  <tfoot className="bg-slate-50 dark:bg-slate-800/50 border-t-2 border-slate-300 dark:border-slate-600">
                    <tr className="font-bold">
                      ...
                      <td className="p-3 text-center">{byProduct.reduce((s, p) => s + p.cases, 0)}</td>
                      <td className="p-3 text-center text-green-600">{byProduct.reduce((s, p) => s + p.bottles, 0)}</td>
                      <td className="p-3 text-center">{byProduct.reduce((s, p) => s + p.bulkLiters, 0).toFixed(1)}</td>
                      <td className="p-3 text-center">{byProduct.reduce((s, p) => s + p.lpLiters, 0).toFixed(1)}</td>
                      <td className="p-3 text-right text-blue-700 dark:text-blue-300">₹{byProduct.reduce((s, p) => s + p.cost, 0).toFixed(2)}</td>
                      <td className="p-3 text-right text-[10px] text-gray-400">100%</td>
                    </tr>
                  </tfoot>
                </table>
